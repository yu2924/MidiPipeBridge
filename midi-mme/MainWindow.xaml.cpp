...
namespace winrt::MidiPipeBridge::implementation
{
	...
	void MainWindow::InitializeComponent()
	{
		...
		// set window position
		static const Windows::Graphics::SizeInt32 EstimatedWindowSize{ 358, 318 };
	...
	Windows::Foundation::IAsyncAction MainWindow::ExportDeviceListAsync(bool foroutputs)
	{
		...
		try
		{
			co_await Windows::Storage::FileIO::WriteTextAsync(file, L"DeviceId" + del + L"DeviceName\n", Windows::Storage::Streams::UnicodeEncoding::Utf8);
			for(const auto& inf : foroutputs ? mainModel.MidiOutDeviceInfoList() : mainModel.MidiInDeviceInfoList())
			{
				if(!MidiDeviceInfo::IsValidDeviceId(inf.DeviceId(), foroutputs)) continue;
				co_await Windows::Storage::FileIO::AppendTextAsync(file, winrt::to_hstring(inf.DeviceId()) + del + inf.DeviceName() + L"\n", Windows::Storage::Streams::UnicodeEncoding::Utf8);
			}
		}
